...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── CONFIGURACIÓN (RBAC) ──
    const MODULO_NAME = 'Principal1.1'; // Ajusta esto al nombre exacto de tu módulo
    ...
    // Aplicar permiso al botón de Nuevo
    ...
    const cargarEstaticos = (pagina = 1, silencioso = false) => {
        paginaActual = pagina;
    ...
    const renderTabla = (items, fromIndex) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            // Lógica idéntica a usuarios: Si no hay permiso, se bloquea o desaparece visualmente
    ...
    // 2. Motor de Tiempo Real (Silent Polling) para los permisos
    setInterval(() => {
        const busquedaActiva = elements.buscador.value.trim() !== '';
        if (paginaActual === 1 && !busquedaActiva) {
            cargarEstaticos(1, true); // true = silencioso, sin skeleton
        }
    }, 4000);
